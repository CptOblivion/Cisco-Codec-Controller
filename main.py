...
class Main():
    def __init__(self):
        ...
        s.Settings.parseBindings(s.Settings.config['Startup']['Bindings'])

        UserPrefabCommandsTemp = s.Settings.config['User Commands'][s.Settings.CustomCommandName].splitlines()
    ...
    def xStatus(self, message, CamNumber = None):
        if (not CamNumber): CamNumber = c.Camera.selectedNum
    ...
    def SendManualCommand(self, entryWidget):
        ...
        message = message.replace(self.PrefabCommandCurrentCamera, 'CameraID:' + str(c.Camera.selectedNum))
        message = message.replace(self.PrefabConfigCurrentCamera, 'Camera ' + str(c.Camera.selectedNum))
    ...
    def toggleAllPresetEditStates(self, state):
        for child in self.Frame_PresetsContainer.contents.winfo_children():
            if (isinstance(child, ui.CameraPresetPanel)):
    ...
    def filterPresetsCurrent(self):
        ...
        for i in range(1, len(self.presetAddButtons)):
            ...
            if (filterCurrentStatus and not c.Camera.selectedNum==i): filtered=True
    ...
    def CreateNewPreset(self, camNum, nameField=None, numField=None):
        ...
        if (camNum is None): camNum=c.Camera.selectedNum
        if (camNum == 0):
            num=''
            if (numField):
                num=numField.get()
    ...
    def InitializePresetLists(self):
        debug.log('(re)initializing presets')
    ...
    def SetPanTilt(self, p, t):
        ...
        if (p is not None and p!= c.Camera.selected.position[0]):
            self.PanningStart()
            sendString += ' Pan: ' + str(p*panRange)
        if (t is not None and t != c.Camera.selected.position[1]):
            self.TiltingStart()
    ...
    def CenterCamera(self):
        ...
        if (c.Camera.selected.position[0] != self.CameraPanCenter): self.PanningStart()
        if (c.Camera.selected.position[1] != self.CameraTiltCenter): self.TiltingStart()
        if (c.Camera.selected.position[2] != self.CameraZoomCenter): self.ZoomingStart()
        #if (c.Camera.selected.position[3] != self.CameraFocusCenter): self.FocusingStart() #check if focus is manual before doing this
    ...
    def OnCameraChange(self, cameraNumber):
        if (cameraNumber != c.Camera.selectedNum): self.FeedbackUpdate(c.Camera.selectedNum, cameraNumber)

        if (self.Frame_PresetsContainer): self.filterPresetsCurrent()
        self.toggleFocusManual.config(variable=c.Camera.selected.focusManual)
        self.toggleBrightnessManual.config(variable=c.Camera.selected.brightnessManual)
        self.ScaleBrightness.setVariable(c.Camera.selected.brightnessValue)
        self.toggleGammaManual.config(variable=c.Camera.selected.gammaManual)
        self.ScaleGamma.setVariable(c.Camera.selected.gammaValue)
        self.toggleWhitebalanceManual.config(variable=c.Camera.selected.whitebalanceManual)
        self.ScaleWhitebalance.setVariable(c.Camera.selected.whitebalanceValue)
        self.UpdateCameraDetails()

    def UpdateCameraDetails(self):
        self.xStatus('Position')
        self.GetCameraConfig(c.Camera.selectedNum)
    ...
    def CameraAvailable(self, cameraNumber, available):
        if (available): self.cameras[cameraNumber].onEnable()
            ...
            if (cameraNumber == c.Camera.selectedNum):
                i=1
    ...
    def UpdateCustomCommandList(self):
        ...
        for command in self.UserPrefabCommands:
            CustomCommandString += '\n' + command
        s.Settings.config['User Commands'][s.Settings.CustomCommandName] = CustomCommandString
    ...
    def SetFocusAuto(self):
        if (c.Camera.selected.focusManual.get()): self.xConfiguration('Focus Mode: Auto')
        else: self.xConfiguration('Focus Mode: Manual')
    def SetBrightnessAuto(self):
        if (c.Camera.selected.brightnessManual.get()): self.xConfiguration('Brightness Mode: Auto')
        else: self.xConfiguration('Brightness Mode: Manual')
    def SetWhitebalanceAuto(sel):
        if (c.Camera.selected.whitebalanceManual.get()): self.xConfiguration('Whitebalance Mode: Auto')
        else: self.xConfiguration('Whitebalance Mode: Manual')
    def SetGammaAuto(self):
        if (c.Camera.selected.gammaManual.get()): self.xConfiguration('Gamma Mode: Auto')
    ...
    def PopulateButtons(self):
        ...
        if True:
            ...
            if True:
                for i in range(7):
                    ...
                    self.cameras[cami].selectButton = tk.Button(self.Frame_CameraList, text=str(cami)+'  ', image=c.Camera.imageCamAvailable, compound='center', command=lambda cami=cami: self.cameras[cami].select())
            ...
            if True:
                ...
                if True:
                    self.TogglePresetEdit = ui.ToggleButtonChecked(Frame_PresetsToolbar,
                ...
                self.Frame_PresetsContainer = ui.ScrollFrame(Frame_Presets, maxHeight=400,
                ...
                #packing of this button happens in c.Camera.onEnable()

                self.presetAddButtons.append(tk.Frame(frame_presetHeader))
                presetNameField=tk.Entry(self.presetAddButtons[0])
                presetNameField.validation=presetNameField.register(ui.CameraPresetPanel.validatePresetName)
            ...
            if True:
                ...
                self.Frame_ConfigPopout = ui.ToggleFrame(Frame_SetupPanel, title='Configuration', keepTitle=True,
                ...
                if True:
                    ...
                    if True:
                        ...
                        if True:
                            self.ScaleBrightness=ui.ConfigSlider(frame_BrightnessButtons,
                    ...
                    if True:
                        ...
                        if True:
                            self.ScaleWhitebalance=ui.ConfigSlider(frame_WhitebalanceButtons,
                    ...
                    if True:
                        ...
                        if True:
                            self.ScaleGamma=ui.ConfigSlider(frame_GammaButtons,
        ...
        Frame_CustomCommandsParent = ui.ToggleFrame(self.window, title='Custom commands', togglePin='left',
                                                 buttonShowTitle='Custom Commands', buttonHideTitle='Hide',
                                                 toggleCommand=weightToggle)
        self.Frame_CustomCommands = ui.ScrollFrame(Frame_CustomCommandsParent.contentFrame, maxHeight=400,
                                                frameConfigureCommand=lambda widget:
        ...
        if True:
            ...
            if True:
                ...
                self.SavedPrefabCommandsView = ui.ToggleFrame(Frame_CustomCommandsToolbar, title='Saved Commands',
    ...
    def settingsMenuClosed(self, event):
        ...
        s.InputRouting.bindListenCancelSafe()
    ...
    def PopulateStartScreen(self):
        def SSHConnect():
            ...
            s.Settings.config['Startup']['IPADDRESS'] = AddressField.get()
            s.Settings.config['Startup']['USERNAME'] = UsernameField.get()
            s.Settings.config['Startup']['PASSWORD'] = PasswordField.get()
        ...
        if True:
            ...
            if True:
                ...
                AddressField.insert(0,s.Settings.config['Startup']['IPADDRESS'])
            ...
            if True:
                ...
                UsernameField.insert(0,s.Settings.config['Startup']['USERNAME'])
            ...
            if True:
                ...
                PasswordField.insert(0,s.Settings.config['Startup']['PASSWORD'])
    ...
    def processController(self):
        ...
        if (len(self.inputDevicesControllers)):
            ...
            for a in range(len(self.inputDevicesControllersLastVals['axis'])):
                binding=s.Settings.commandBinds['controller']['axis'][a]
                if (self.SettingsMenu):
                    ...
                    if (s.InputRouting.settingsListenForInput):
                        ...
                        if (value > triggerThreshold and not (-stickThreshold<value<stickThreshold)):
                            ...
                            s.InputRouting.bindCommand('controller', 'axis', 'analog',
            ...
            for b in range(len(self.inputDevicesControllersLastVals['button'])):
                if (self.SettingsMenu):
                    ...
                    if (changed and value and s.InputRouting.settingsListenForInput):
                         s.InputRouting.bindCommand('controller', 'button', 'button', [b])
                elif (s.Settings.commandBinds['controller']['button'][b]):
                    changed, value = checkButton(b)
                    if (changed):
                        s.Settings.commandBinds['controller']['button'][b].callCommand(value)
                ...
                if (hat != lastVal):
                    if (self.SettingsMenu):
                        ...
                        if (hat is not None and s.Settings.commandBinds['controller']['hat'][h][hat]):
                            s.Settings.commandBinds['controller']['hat'][h][hat].callCommand(True)
